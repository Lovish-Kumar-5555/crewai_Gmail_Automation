...
from gmail_crew_ai.guardrails import (
...
def _find_email_context(email_id: Optional[str] = None, subject: Optional[str] = None) -> Dict[str, Any]:
    """
    Best-effort metadata lookup to support guardrail checks even when the agent omits fields.
    """
    candidates: list[dict[str, Any]] = []
    for path in ("output/categorization_report.json", "output/fetched_emails.json"):
    ...
    for item in candidates:
        item_id = str(item.get("email_id", "")).strip()
        item_subject = str(item.get("subject", "")).strip().lower()
        if email_id and item_id and item_id == email_id:
            return item
        if subject and item_subject and item_subject == subject:
            return item
    return {}
...
class SaveDraftSchema(BaseModel):
    ...
    email_id: Optional[str] = Field(None, description="Source email ID for policy checks")
    category: Optional[str] = Field(None, description="Email category for policy checks")
    priority: Optional[str] = Field(None, description="Email priority for policy checks")
    age_days: Optional[int] = Field(None, description="Email age in days for policy context")
...
class SaveDraftTool(BaseTool):
    ...
    def _run(
        ...
        context = _find_email_context(email_id=email_id, subject=subject)
        validation = validate_action(
            PolicyValidationInput(
                email_id=email_id or context.get("email_id"),
                subject=subject,
                sender=context.get("sender"),
                category=category or context.get("category"),
                priority=priority or context.get("priority"),
                requested_action="draft_reply",
                age_days=age_days if age_days is not None else context.get("age_days"),
                classification_confidence=context.get("classification_confidence"),
                metadata={"recipient": recipient},
            )
        )
        if not validation.allowed:
            log_validation(validation, tool_name=self.name, executed=False)
            return json.dumps(
                validation_response_payload(
        ...
        try:
            ...
            if verified:
                result_text = f"VERIFIED: Draft email saved with subject: '{subject}' in folder {folder}"
            else:
                ...
                try:
                    ...
                    if result == 'OK':
                        result_text = f"Draft saved to All Mail with subject: '{subject}' (flagged as draft)"
                    else:
                        result_text = f"WARNING: Draft save attempt returned {result}, but verification failed. Please check your Gmail Drafts folder."
                except Exception as e:
                    result_text = f"WARNING: Draft may not have been saved properly: {str(e)}"

        except Exception as e:
            result_text = f"Error saving draft: {str(e)}"
        finally:
            if mail is not None:
                self._disconnect(mail)

        executed = not str(result_text).lower().startswith("error")
        log_validation(validation, tool_name=self.name, executed=executed)
        return json.dumps(
            validation_response_payload(
                proposed_action="draft_reply",
                result=validation,
...
class GmailOrganizeTool(GmailToolBase):
    ...
    def _run(self, email_id: str, category: str, priority: str, should_star: bool = False, labels: List[str] = None) -> str:
        ...
        try:
            ...
            result_text = f"Email organized: Starred={should_star}, Labels={labels}"

        except Exception as e:
            result_text = f"Error organizing email: {e}"
        finally:
            self._disconnect(mail)
...
class GmailDeleteSchema(BaseModel):
    ...
    category: Optional[str] = Field(None, description="Email category for policy checks")
    priority: Optional[str] = Field(None, description="Email priority for policy checks")
    age_days: Optional[int] = Field(None, description="Age in days for delete rules")
    sender: Optional[str] = Field(None, description="Sender for sensitivity checks")
    subject: Optional[str] = Field(None, description="Subject for sensitivity checks")
    classification_confidence: Optional[float] = Field(
        None,
        description="Classifier confidence (0-1) when available",
    )
        ...
        try:
            ...
            validation = validate_action(
                PolicyValidationInput(
                    ...
                    sender=sender or context.get("sender"),
                    category=category or context.get("category"),
                    priority=priority or context.get("priority"),
                    ...
                    classification_confidence=(
                        ...
                        else context.get("classification_confidence")
                    ),
            ...
            try:
                ...
                result_text = f"Email deleted: '{subject}' from {sender}. Reason: {reason}"
            except Exception as e:
                result_text = f"Error deleting email: {e}"
            finally:
                self._disconnect(mail)

        except Exception as e:
            result_text = f"Error deleting email: {str(e)}"
...
class EmptyTrashTool(BaseTool):
    ...
    def _run(self) -> str:
        ...
        validation = validate_action(
            PolicyValidationInput(
                requested_action="empty_trash",
                message_ref="gmail_trash",
            )
        )
        if not validation.allowed:
            log_validation(validation, tool_name=self.name, executed=False)
        ...
        try:
                    ...
                    if result == 'OK':
                        ...
                        if result == 'OK':
                            ...
                            if count == 0:
                                ...
                                result_text = "Trash is already empty. No messages to delete."
                                log_validation(validation, tool_name=self.name, executed=True)
                                return json.dumps(
                                    validation_response_payload(
                                        proposed_action="empty_trash",
                                        result=validation,
            ...
            if success:
                result_text = f"Successfully emptied Gmail trash folder ({trash_folder_used}). Deleted {count} messages."
            else:
                result_text = "Could not empty trash. No trash folder found or accessible."

        except Exception as e:
            result_text = f"Error emptying trash: {str(e)}"
        finally:
            if mail is not None:
                self._disconnect(mail)
